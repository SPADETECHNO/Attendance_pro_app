...
class DatabaseService {
  ...
  Future<List<InstituteModel>> getInstitutes() async {
    try {
      ...
      return data
          .map((json) => InstituteModel.fromJson(json as Map<String, dynamic>))
          .toList();
    } catch (e) {
  ...
  Future<void> updateInstitute({
    ...
    try {
      await _client.from(AppConstants.institutesTable).update({
        'name': name,
        'address': address,
        'phone': phone,
        'allowed_radius': allowedRadius,
  ...
  Future<void> deleteInstitute(String id) async {
    try {
      await _client.from(AppConstants.institutesTable).delete().eq('id', id);
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getInstituteAdmins(
...
// ================== INSTITUTE MASTER LIST ==================
  ...
  Future<List<InstituteMasterListModel>> getInstituteMasterList(
      String instituteId) async {
    try {
      final response = await _client
  ...
  Future<void> updateInstituteMasterListUser({
    ...
    try {
      await _client.from('institute_master_list').update({
        'name': name,
        'email': email,
        'phone': phone,
        'department_id': departmentId,
  ...
  Future<void> deleteFromInstituteMasterList(String id) async {
    try {
      await _client.from('institute_master_list').delete().eq('id', id);
    } catch (e) {
      ...
      if (sendEmailInvitations) {
        ...
        for (final user in users) {
          try {
            ...
            AppHelpers.debugError(
                'Auth creation failed for user ${user['user_id']}: $authError');
          }
        }
      }
    } catch (e) {
  ...
  Future<List<DepartmentModel>> getDepartmentsByInstitute(
      String instituteId) async {
    try {
      final response = await _client
  ...
  Future<List<Map<String, dynamic>>> getDepartmentAdmins(
      String instituteId) async {
    try {
      ...
      if (response is List) {
        ...
        for (int i = 0; i < response.length; i++) {
          ...
          print(
              '[DBG] Processing admin $i: ${admin['name']} (${admin['email']})');
          ...
          if (admin['department_id'] != null) {
            try {
              print(
                  '[DBG] Looking up department for ID: ${admin['department_id']}');
  ...
  Future<List<Map<String, dynamic>>> getDepartmentAdminsByAcademicYear(
      String instituteId, String academicYearId) async {
    try {
      ...
      if (response is List && response.isNotEmpty) {
        ...
        print(
            'No admins found for academic year $academicYearId, getting all admins');
  ...
  Future<List<SessionModel>> getUserSessions(String userId) async {
    try {
      final response =
          await _client.from(AppConstants.sessionParticipantsTable).select('''
            sessions!session_id(*)
          ''').eq('user_id', userId);
  ...
  Future<String> createSessionWithParticipants({
    ...
    try {
      ...
      if (participantUserIds != null && participantUserIds.isNotEmpty) {
        final participantRecords = participantUserIds
            .map((userId) => {
  ...
  Future<ParticipantValidationResult> validateParticipants({
    ...
    try {
      ...
      final masterList =
          existingUsers.map((user) => user['user_id'] as String).toSet();
  ...
  Future<void> setCurrentAcademicYear({
    ...
    try {
      ...
      await _client.from(AppConstants.academicYearsTable).update({
        'is_current': false,
        'updated_at': DateTime.now().toIso8601String()
      }).eq('institute_id', instituteId);
      ...
      AppHelpers.debugLog(
          'Successfully set academic year $yearId as current for institute $instituteId');
    } catch (e) {
  ...
  Future<void> deleteAcademicYear(String yearId) async {
    try {
      ...
      if ((sessionsCount as List).isNotEmpty) {
        throw Exception(
            'Cannot delete academic year that has existing sessions');
      }
  ...
  Future<void> createAcademicYear({
    ...
    try {
      ...
      if (isCurrent) {
        await _client
            .from(AppConstants.academicYearsTable)
            .update({'is_current': false}).eq('institute_id', instituteId);
      }
  ...
  Future<int> getSessionCountByDepartmentAndYear(
      String departmentId, String academicYearId) async {
    try {
      final response = await _client
          .from(AppConstants.sessionsTable)
          .select()
      ...
      AppHelpers.debugError(
          'Get session count by department and year error: $e');
      return 0;
    }
  }
  ...
  Future<void> deleteDepartment(String id) async {
    try {
      await _client.from(AppConstants.departmentsTable).delete().eq('id', id);
    } catch (e) {
  ...
  Future<void> deleteAdmin(String adminId) async {
    try {
      await _client.from(AppConstants.profilesTable).delete().eq('id', adminId);
    } catch (e) {
  ...
  /// Get session with all participant details and their attendance status
  Future<List<Map<String, dynamic>>> getSessionParticipantsWithStatus(
      String sessionId) async {
    try {
      final response = await _client.from('session_participants').select('''
          id,
          user_id,
          profiles!inner(
            id,
        ...
        ''').eq('session_id', sessionId).order('added_at', ascending: true);

      return List<Map<String, dynamic>>.from(response);
    } catch (e) {
      AppHelpers.debugError('Get session participants with status error: $e');
      rethrow;
    }
  }

  /// Add user to session during attendance marking
  Future<void> addParticipantToSession({
    required String sessionId,
    required String userId,
    bool addedDuringSession = true,
  }) async {
    try {
      ...
      if (existing != null) {
        throw Exception('User is already a participant in this session');
      }
      ...
      // Create attendance record with 'not_marked' status
      await _client.from('attendance').insert({
        'session_id': sessionId,
        'user_id': userId,
        'marked_by': userId, // Self for now
        'marked_by_user': false,
        'marked_by_admin': true,
        'status': 'not_marked',
        'marked_at': DateTime.now().toIso8601String(),
        'added_during_session': addedDuringSession,
      });
    } catch (e) {
      AppHelpers.debugError('Add participant to session error: $e');
      rethrow;
    }
  }

  /// Search users in master list for adding to session
  Future<List<Map<String, dynamic>>> searchMasterListUsers({
    required String instituteId,
    String? query,
    String? departmentId,
    String? academicYearId,
    ...
    try {
      var queryBuilder = _client.from('institute_master_list').select('''
          id,
          user_id,
          name,
          email,
        ...
        ''').eq('institute_id', instituteId).eq('account_status', 'active');
      ...
      if (academicYearId != null) {
        queryBuilder = queryBuilder.eq('academic_year_id', academicYearId);
      }

      if (query != null && query.isNotEmpty) {
        queryBuilder = queryBuilder.or(
            'user_id.ilike.%$query%,name.ilike.%$query%,email.ilike.%$query%');
      }
      ...
      AppHelpers.debugError('Search master list users error: $e');
      rethrow;
    }
  }

  /// Get session statistics
  Future<Map<String, int>> getSessionStatistics(String sessionId) async {
    try {
      final attendanceRecords = await getAttendanceRecords(sessionId);
      ...
      final notMarked =
          attendanceRecords.where((a) => a.status == 'not_marked').length;
      ...
      AppHelpers.debugError('Get session statistics error: $e');
      rethrow;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getAttendanceReportData(
      String sessionId) async {
    try {
      final response = await _client.from('attendance').select('''
          id,
          status,
          marked_at,
          marked_by_user,
      ...
      AppHelpers.debugError('Get attendance report data error: $e');
      rethrow;
    }
  }
...
// lib/services/database_service.dart
  ...
  /// Add user to master list (simplified - no auth)
  Future<void> addToInstituteMasterList({
    ...
    try {
      ...
      await client.from('institute_master_list').insert({
        'user_id': userId,
        'name': name,
        'email': email,
        'phone': phone,
      ...
      AppHelpers.debugError('Add to master list error: $e');
      rethrow;
    }
  }
// lib/services/database_service_attendance.dart
// ⭐ NEW FILE - Add these methods to your existing DatabaseService

// Add to your existing DatabaseService class:

  /// ⭐ SIMPLIFIED: Create session with attendance records
  Future<String> createSessionWithAttendance({
    required String name,
    String? description,
    required DateTime startDateTime,
    required DateTime endDateTime,
    ...
    required String departmentId,
    required String createdBy,
    required bool gpsValidationEnabled,
    required List<String> masterListIds, // ⭐ Direct master list IDs
  }) async {
    try {
      // 1. Create session
      ...
      // 2. Create attendance records for all participants
      final attendanceRecords = masterListIds.map((masterListId) {
        return {
          'session_id': sessionId,
      ...
      AppHelpers.debugError('Create session with attendance error: $e');
      rethrow;
    }
  }

  /// ⭐ SIMPLIFIED: Mark attendance
  Future<Map<String, dynamic>> markAttendanceSimplified({
    required String sessionId,
    required String masterListId, // ⭐ Changed from userId to masterListId
    required String markedBy,
    required String status,
    double? gpsLatitude,
    double? gpsLongitude,
    ...
    try {
      ...
      if (existing != null) {
        ...
        final newScanCount = (existing['scan_count'] as int? ?? 0) + 1;

        await client.from(AppConstants.sessionAttendanceTable).update({
          'status': status,
          'marked_at': DateTime.now().toIso8601String(),
          'marked_by': markedBy,
        ...
        // New record - user added during session
        await client.from(AppConstants.sessionAttendanceTable).insert({
          'session_id': sessionId,
          'master_list_id': masterListId,
          'status': status,
          'marked_at': DateTime.now().toIso8601String(),
  ...
  /// ⭐ SIMPLIFIED: Get session attendance
  Future<List<Map<String, dynamic>>> getSessionAttendanceSimplified(
    String sessionId,
  ) async {
    try {
      final response =
          await client.from(AppConstants.sessionAttendanceTable).select('''
          id,
          session_id,
          master_list_id,
          status,
  ...
  /// ⭐ SIMPLIFIED: Get session statistics
  Future<Map<String, int>> getSessionStatisticsSimplified(
    String sessionId,
  ) async {
    try {
      final records = await getSessionAttendanceSimplified(sessionId);

      final total = records.length;
      final present = records.where((r) => r['status'] == 'present').length;
      final absent = records.where((r) => r['status'] == 'absent').length;
      final notMarked =
          records.where((r) => r['status'] == 'not_marked').length;
  ...
  Future<void> updateSession({
    ...
    try {
      ...
      if (gpsValidationEnabled != null) {
        updates['gps_validation_enabled'] = gpsValidationEnabled;
      }
      ...
      AppHelpers.debugLog('Session updated: $sessionId');
    } catch (e) {
  ...
  /// Get session participants (master list IDs)
  Future<List<String>> getSessionParticipantIds(String sessionId) async {
    try {
      ...
      return (response as List)
          .map((r) => r['master_list_id'] as String)
          .toList();
    } catch (e) {
      AppHelpers.debugError('Get session participants error: $e');
      rethrow;
    }
  }

  /// Add participants to existing session
  Future<void> addSessionParticipants({
    required String sessionId,
    ...
    try {
      ...
      AppHelpers.debugLog('Added ${masterListIds.length} participants to session');
    } catch (e) {
      AppHelpers.debugError('Add session participants error: $e');
      rethrow;
    }
  }

  /// Remove participants from session
  Future<void> removeSessionParticipants({
    required String sessionId,
    required List<String> masterListIds,
  }) async {
    try {
      await client
          ...
          .delete()
          .eq('session_id', sessionId)
          .inFilter('master_list_id', masterListIds);

      AppHelpers.debugLog('Removed ${masterListIds.length} participants from session');
    } catch (e) {
      AppHelpers.debugError('Remove session participants error: $e');
      rethrow;
    }
  }
  ...
  Future<void> deleteSession(String sessionId) async {
    try {
      ...
      AppHelpers.debugLog('Session deleted: $sessionId');
    } catch (e) {
  ...
  /// ⭐ SIMPLIFIED: Search users by user_id (for QR/manual entry)
  Future<Map<String, dynamic>?> searchMasterListByUserId({
    required String instituteId,
    required String userId, // e.g., "202411073"
  }) async {
    try {
      final response = await client
          .from('institute_master_list')
